...
--Load our 1st model in gdx/g3dj format, and specify the format
local mesh=buildGdx("models/characterLargeFemale/characterLargeFemale.json",
	{ modelpath="models/characterLargeFemale/", } -- don't forget the / at the end
)
...
D3Anim.setAnimation(mesh,animIdle.animations[2],"main",true)

--Load our 2nd more complex model (from mixamo) in gdx/g3dj format
local mesh2=buildGdx("models/theBoss/theBoss_mesh.json",
	{ modelpath="models/theBoss/", }
)
mesh2:setScale(0.015,0.015,0.015)
local animIdle2=buildGdx("models/theBoss/theBoss_old_man_anim.json",{})
D3Anim.setAnimation(mesh2,animIdle2.animations[1],"main",true)

-- position
mesh:setPosition(0, 0, 0)
mesh2:setPosition(0, 0, 1.2)
-- order
...
stage:addEventListener(Event.MOUSE_DOWN,function()
	D3Anim.setAnimation(mesh,animWalk.animations[2],"main",true,0.5)
	walk=true
end)
stage:addEventListener(Event.MOUSE_UP,function()
	D3Anim.setAnimation(mesh,animIdle.animations[2],"main",true,0.5)
	walk=false
end)
...
stage:addEventListener(Event.ENTER_FRAME,function(e)
	...
	mesh:setRotationY(wang+90)
	mesh:setPosition(as,0.2,ac)
