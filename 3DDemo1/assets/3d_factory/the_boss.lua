TheBoss = Core.class(Sprite)

function TheBoss:init(xworld, xcamera, xparams)
	-- the params
	...
	self.camera = xcamera
	-- the mesh
	local xfilemesh = "models/the_boss/the_boss_mesh.json"
	local xfileidle = "models/the_boss/the_boss_old_man_anim.json"
	-- Load our model in gdx/g3dj format and its texture
	self.mesh = buildGdx(xfilemesh,
		{
			modelpath="models/the_boss/",
		}
	)
	-- scale it down
	local meshscale = 0.012
	...
	print("the boss", width, height, depth)
...
--	self.body:setAngularVelocity(0.1, 0.1, 0.1) -- 0.9 play with it!
	...
	-- the shape
	self.shape = r3d.BoxShape.new(width/5, height/2, depth)
...
-- GAME LOOP
function TheBoss:onEnterFrame(e)
	-- move player
	local matrix = self.body:getTransform()
